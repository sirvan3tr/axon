//
// Created by Sirvan Almasi on 28/01/2022.
//

#ifndef AXON_LIS3DH_H
...
class LIS3DH {
  public:
    ...
    boolean data_ready(void); // check LIS3DH_DRDY pin
    boolean data_available(void); // check LIS3DH STATUS_REG2
    void    read_all_regs(void);
    void    writeAxisDataSerial(void);
    void    writeAxisDataForAxisSerial(uint8_t);
    void    updateAxisData(void);
    void    zero_axis_data(void);
};
